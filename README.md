...
This forwarding mail service forked from [anonaddy/docker](https://github.com/anonaddy/docker)  
This setup use the **Traefik** example to setup the mail service and web app.

The mail service will not work on cloud provider (like Digitalocean) that block port 25 by default since they may try to protect their Ip from spams...
...
# check if traefik and the other services work as intended
sudo docker compose -f


## conveniance
clear && sudo docker compose down && sudo docker compose up -d --remove-orphans && sudo docker compose logs -f
```
